...
def register_qmc(y, x, w, **kwargs):
    """Register QMC layer tag for KFAC optimization.

    Args:
        y: Output tensor
        x: Input tensor
        w: Weight tensor
        **kwargs: Additional arguments for tag binding

    Returns:
        Tagged layer for KFAC
    """
    return qmc_tag.bind(y, x, w, **kwargs)


def register_repeated_dense(y, x, w, b):
    """Register repeated dense layer tag for KFAC optimization.
...
class RepeatedDenseBlock(kfac_jax.DenseTwoKroneckerFactored):
    ...
    def fixed_scale(self) -> Numeric:
        """Calculate fixed scale factor for the block."""
    ...
    def update_curvature_matrix_estimate(
        ...
        """Update curvature matrix estimate for repeated dense block."""
...
class QmcBlockedDense(kfac_jax.TwoKroneckerFactored):
    ...
    def input_size(self) -> int:
        """Return input size (not implemented)."""
        raise NotImplementedError()

    def output_size(self) -> int:
        """Return output size (not implemented)."""
        raise NotImplementedError()

    def fixed_scale(self) -> Numeric:
        """Calculate fixed scale factor from parameter shapes."""
    ...
    def update_curvature_matrix_estimate(
        ...
        """Update curvature matrix estimate for QMC blocked dense layer."""
        (x,) = estimation_data["inputs"]
        (dy,) = estimation_data["outputs_tangent"]
    ...
    def multiply_matpower(
        ...
        """Multiply vector by matrix power."""
        (w,) = vector
        # kmjn
        v = w
        k, m, j, n = v.shape
...
repeated_dense1_with_bias_pattern = kfac_jax.tag_graph_matcher.GraphPattern(
    ...
    example_args=[
        ...
        [np.zeros([13, 7], dtype=np.float32), np.zeros([7], dtype=np.float32)],
    ],
)
...
repeated_dense1_no_bias_pattern = kfac_jax.tag_graph_matcher.GraphPattern(
    ...
    example_args=[
        np.zeros([9, 11, 13], dtype=np.float32),
        [np.zeros([13, 7], dtype=np.float32)],
    ],
)
...
repeated_dense2_with_bias_pattern = kfac_jax.tag_graph_matcher.GraphPattern(
    ...
    parameters_extractor_func=_dense_parameter_extractor,
    example_args=[
        np.zeros([8, 9, 11, 13], dtype=np.float32),
        [np.zeros([13, 7], dtype=np.float32), np.zeros([7], dtype=np.float32)],
    ],
)
